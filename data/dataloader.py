    ...
    def __init__(self, data_path, annotation_file, time_window):
        ...
        if not os.path.exists(os.path.join(data_path, "audio.pkl")):
            ...
            for i, row in self.annotation_data.iterrows():
                ...
                segment_audio = audio_data[(cut_time - time_window // 2) * sample_rate: (cut_time + time_window // 2) * sample_rate, :]
...
class VideoDataset(Dataset):
    def __init__(self, data_path, annotation_file, time_window, N):
        self.annotation_data = pd.read_csv(annotation_file)
        self.data_path = data_path
        self.N = N
        self.labels = []
        ...
        if not os.path.exists(os.path.join(data_path, "video.pkl")):
            ...
            temp_film = self.annotation_data["Film"][0]
            frames = video.get_frames(os.path.join(data_path, temp_film + ".mp4"), width=32, height=32)
...
if __name__ == "__main__":
    ...
    audio_video_dataset = AudioVideoDataset("../../Data/RedHenLab/Color Films/EditedColorFilms/",
                                            "../../Data/RedHenLab/Color Films/EditedColorFilms/ColorFilmsAnnotations.csv",
                                            8, 64)

    audio_video_loader = DataLoader(audio_video_dataset, batch_size=8, shuffle=True)

    for x, y, labels in audio_video_loader:
        print(x.shape)
        print(y.shape)
